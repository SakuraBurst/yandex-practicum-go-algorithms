...
-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
В целом временная сложность равна O(n * log n): мы добавим все элементы в порядке их приоритета за O(n * log n), потом вернем
все элеметны попутно перебалансируюя кучу за O(n * log n), получается O(n * log n + n * log n) = O(n * log n)

ps
в теории временная сложность описывалась как O(1)+O(n * log n)+O(n * log n)=O(n * log n), но, к сожалению, я так и не понял, что в этой соритровке
равно O(1)
