...
Привет! В общем-то задача была не сложная, я использовал алгоритм Дийскстры для постриоения максимального остовго дерева и
...
Далее мы начинаем брать вершины из очередь пока не найдем непосещенную и, когда находим, инкерементим специальную пременную для того,
чтобы вернуть общую длинну пути. Мы кладем каждую непосещенную вершину, к которой есть путь из текущей вершины, в очередь и помечаем текущую вершину
как посещенную.
Алгорим повторяется до тех пор, пока в куча не станет пустой. Благодарая этому мы получаем максимальную длинну пути. После того как алгорим отработал,
мы проходимся по массиву visited, чтобы проверить, что все вершины посещены, если все посещены, отправляем длинну пути, если нет, то кодовую фразу
...
что у нас в куче может лежать больше одного пути к одной и той же ноде.
...
visited, это еще + |V|, но сокращая асимптоматика остается прежней.
